...
impl File {
    ...
    /// # Panics
    ///
    /// Panics if the index is not in the range `0..=6`.
    #[inline]
...
#[repr(u8)]
pub enum Square {
   _A1=0, _B1,   _C1,   D1,   E1,  F1,  _G1, _H1,
   _A2,   _B2,    C2,   D2,   E2,  F2,   G2, _H2,
   _A3,    B3,    C3,   D3,   E3,  F3,   G3,  H3,
   _A4,    B4,    C4,   D4,   E4,  F4,   G4,  H4,
   _A5,    B5,    C5,   D5,   E5,  F5,   G5,  H5,
   _A6,   _B6,    C6,   D6,   E6,  F6,   G6, _H6,
   _A7,   _B7,   _C7,   D7,   E7,  F7,  _G7, _H7,
...
impl Square {
    ...
    /// use shakmaty::Square;
    ///
    /// assert_eq!(Square::F3.offset(7), Some(Square::F4));
...
mod tests {
    ...
    #[test]
    fn test_square() {
        for file in (0..7).map(File::new) {
            for rank in (0..7).map(Rank::new) {
